...
class RemoteControlHandler:
    ...
    def register_handlers(self):
        ...
        def handle_sio_connect():
            ...
            emit("connection_acknowledged", {"status": "connected", "sid": client_sid}, room=client_sid)
        ...
        def handle_sio_disconnect():
            ...
            if client_sid in self.connected_clients_sio:
                ...
                self.socketio.emit("client_disconnected", {"device_id": dev_id_display, "sid": client_sid})
            else:
                logger.warning(
        ...
        def handle_device_heartbeat(data):
            ...
            if client_sid in self.connected_clients_sio:
                ...
                self.connected_clients_sio[client_sid]["last_seen"] = datetime.datetime.now().isoformat()
                
                # Update additional status information if provided
                if isinstance(data, dict):
                    if "status" in data:
                        self.connected_clients_sio[client_sid]["status"] = data["status"]
                    if "battery" in data:
                        self.connected_clients_sio[client_sid]["battery"] = data["battery"]
                    if "network" in data:
                        self.connected_clients_sio[client_sid]["network"] = data["network"]
                
                # Send heartbeat acknowledgment for better client synchronization
                emit("heartbeat_ack", {"timestamp": datetime.datetime.now().isoformat()}, room=client_sid)
                
                # Notify admin clients about updated status
                self.socketio.emit("client_status_update", {
                    "sid": client_sid,
                    "device_id": self.connected_clients_sio[client_sid]["id"],
                    "last_seen": self.connected_clients_sio[client_sid]["last_seen"],
                    "status": self.connected_clients_sio[client_sid].get("status", "active")
                })
            else:
                logger.warning(
        ...
        def handle_command_response(data):
            ...
            if device_info:
                if status == "success":
                    device_info["commands_succeeded"] = device_info.get("commands_succeeded", 0) + 1
                elif status in ["error", "failed"]:
                    device_info["commands_failed"] = device_info.get("commands_failed", 0) + 1
    ...
    def send_command_to_client(self, target_sid, command_name, args=None, timeout=None):
        ...
        if target_sid in self.connected_clients_sio:
            ...
            command_id = f"{command_name}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            ...
            self.socketio.emit(command_name, args, to=target_sid)
    ...
    def _monitor_command_timeout(self, command_id, timeout):
        ...
        if command_id in self.command_queue:
            ...
            if target_sid in self.connected_clients_sio:
                client_info = self.connected_clients_sio[target_sid]
                client_info["commands_failed"] = client_info.get("commands_failed", 0) + 1
            
            # Notify admin clients about timeout
            self.socketio.emit("command_timeout", {
                "command_id": command_id,
                "command": command_name,
                "device_id": device_id,
                "timeout": timeout,
...
def register_remote_control_routes(app, remote_control_handler):
    ...
    def send_command(sid):
        ...
        try:
            ...
            result = remote_control_handler.send_command_to_client(sid, command, args, timeout)
